...
- **upload-to-quay**
    - Collects all built artifacts from previous steps and uploads them to
      Quay.io (the destination is selected by the user as a pipeline parameter).
- **check-noarch**
    - Verifies that all noarch (sub-)packages from the architecture-specific
      builds are identical.  Noarch sub-packages are built on all architectures,
      but typically, we want to de-duplicate and distribute only one of them.
